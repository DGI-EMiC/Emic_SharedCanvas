...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <div xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/"
            ...
            <xsl:for-each select="/rdf:RDF/rdf:Description[rdf:type/@rdf:resource ='http://www.openannotation.org/ns/Annotation']">
                <div>
                    ...
                    <xsl:for-each select="/rdf:RDF/rdf:Description[rdf:type/@rdf:resource ='http://www.w3.org/2008/content#ContentAsText']">
                        <xsl:choose>
                            <xsl:when test="./rdf:type/@rdf:resource = 'http://www.openannotation.org/ns/SvgConstraint'">
                            </xsl:when>
                            <xsl:otherwise>
                                <div>
                                    <xsl:attribute name="about">
                    ...
                    <xsl:for-each select="/rdf:RDF/rdf:Description[rdf:type/@rdf:resource ='http://www.openannotation.org/ns/ConstrainedTarget']">
                        <div>
                            <xsl:attribute name="about">
                                <xsl:value-of select="./@rdf:about"/>
                            </xsl:attribute>
                            <xsl:call-template name="process_div" />
                            <xsl:for-each select="/rdf:RDF/rdf:Description[rdf:type/@rdf:resource ='http://www.openannotation.org/ns/SvgConstraint']">
                                <div>
                                    <xsl:attribute name="about">
                                        <xsl:value-of select="./@rdf:about"/>
                                    </xsl:attribute>
