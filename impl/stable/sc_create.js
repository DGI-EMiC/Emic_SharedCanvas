...
var outsideStyle = {fill: '#FFBBFF', opacity: 0.7, 'stroke-width': 2, stroke: 'black'};
var insideStyle = {fill: '#FFFFFF', opacity: 0.3, 'stroke-width': 2, stroke: 'black', 'stroke-dasharray': '- '};

function fetch_comment_annotations() {
	// So far only pastebin via proxy
	pb_getList();
	// NB gdata will be fetched on login
}
...
function closeAndEndAnnotating() {
	...
	$('#create_annotation').css({color:'#000000'});
	$('#canvases .canvas').each(function() {
...
function initForCreate(canvas) {
	...
	bg.attr({'fill': 'white', 'opacity': 0.15});
	// bg.toBack();
	bg.creating = null;
...
function saveAnnotation() {
	...
	if (which == 'pb_pastebin') {
		var data = $(rdfa).rdf().databank.dump({format:'text/turtle',serialize:true});
...
function create_rdfAnno() {
	...
	var typ = null;
	$('#anno_type :selected').each(function() {typ = this.value;});
...
switchUp = function(x, y) {
	var which = topinfo['svgAnnoShape'];
	if (which == 'circ') {
		this.creating.start=[]; this.creating = null;
	} else if (which == 'rect') {
		this.creating.set.start=[]; this.creating = null;
	}	
}
...
function mkGrabber(what,poly,x,y,idx) {
	...
	var mdf = function() { this.moved = 0; this.start = [this.attr("cx"), this.attr("cy")]};
	...
	var move = function (dx, dy) { 
		...
		this.attr({cx: this.start[0] + dx, cy: this.start[1] + dy})
...
function mkPoly(what, x,y) {
	...
	var mdf = function() {this.set.tmp = [0,0];};
	var muf = function() {this.set.tmp = undefined;};
	var move = function(dx,dy) {
		dx=Math.floor(dx * what.invScale);
		dy=Math.floor(dy * what.invScale);
	...
	outer.dblclick(function() {this.set.remove();});
	return outer;
}
...
function mkCircle(what, x,y) {
	...
	muf = function() {this.start = undefined;};
	...
	inner.dblclick(function() {this.set.remove();});
	return outer;
}
...
function mkRect(what, x,y) {
	...
	muf = function() {this.set.start = undefined;};
